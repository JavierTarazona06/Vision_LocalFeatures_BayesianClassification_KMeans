...
\subsection{Classiﬁcation bayésienne des pixels}
...
\subsubsection{Modification du mod\`ele d'entra\^inement}
...
Pour le protocole recommand\'e dans ce rapport, nous sugg\'erons d'annoter \texttt{img/Essex\_Faces/94/asamma.19.jpg} comme image d'apprentissage, puis de tester le mod\`ele appris sur \texttt{img/Essex\_Faces/94/ajones.19.jpg} et \texttt{img/Essex\_Faces/96/arwebb.19.jpg}. Un jeu d'annotations raisonnable consiste \`a s\'electionner environ 5 \`a 8 RoI de peau et 5 \`a 8 RoI de non-peau, en veillant \`a inclure parmi les n\'egatifs le fond, les cheveux, les yeux, les sourcils et les zones d'ombre. Ce choix limite le risque que la composante de gradient favorise \`a tort des contours marqu\'es qui ne correspondent pas \`a la peau.
